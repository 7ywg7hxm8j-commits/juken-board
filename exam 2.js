...
// =============================================
const EXAM_SERIES = [
  { id: 'kawaijuku-zento',  name: '河合塾 全統模試',       icon: '📘' },
  { id: 'sundai-zento',     name: '駿台 全国模試',         icon: '📗' },
  { id: 'benesse-gt',       name: 'ベネッセ GTZ模試',      icon: '📙' },
  { id: 'toshin-center',    name: '東進 センター模試',     icon: '📕' },
  { id: 'kawaijuku-kyotsu', name: '河合塾 共通テスト模試', icon: '📓' },
  { id: 'sundai-kyotsu',    name: '駿台 共通テスト模試',   icon: '📔' },
...
function renderSeriesList() {
  ...
  if (!el) return;
  el.innerHTML = EXAM_SERIES.map(s => `
    <div class="card" onclick="Exam.enterSeries('${s.id}','${esc(s.name)}')">
...
async function loadRooms() {
  ...
  if (rooms.length === 0) {
    grid.innerHTML = `<div class="empty-state"><div class="icon">📭</div>
      <p>まだ部屋がありません<br>「＋ 回を追加する」から作れます</p></div>`;
    return;
  }
  grid.innerHTML = rooms.map(r => `
    <div class="card" onclick="Exam.enterRoom('${r.id}','${esc(r.name)}')">
...
async function loadSubjects() {
  ...
  if (subjects.length === 0) {
    grid.innerHTML = `<div style="font-size:0.84rem;color:#bbb;padding:12px 0">
      まだ教科がありません。下のボタンから追加してください。</div>`;
    return;
  }
  grid.innerHTML = subjects.map(s => `
    <div class="list-card" onclick="Exam.enterSubject('${s.id}','${esc(s.name)}')">
      <div>
        <div class="list-card-title">${esc(s.name)}</div>
        <div class="list-card-sub">回答 ${s.answerCount||0}件</div>
      </div>
  ...
  if (schema.length === 0) {
    container.innerHTML = `
      ...
      <button class="btn-primary" style="margin-top:16px;width:100%"
        onclick="Exam.showSchemaModal()">＋ 問題の型を作成する</button>`;
    return;
  }
...
function buildAnswerFormHTML(schema, myAnswers, allAnswers) {
  ...
  let html = `
    <div style="display:flex;justify-content:space-between;align-items:center;
      ...
      <button class="btn-ghost" style="font-size:0.78rem;padding:6px 12px"
        onclick="Exam.showSchemaModal()">型を編集</button>
    </div>`;

  schema.forEach((daimo, di) => {
    html += `<div style="margin-bottom:28px;">
      <div style="font-family:'Zen Kaku Gothic New',sans-serif;font-weight:900;font-size:1rem;
        color:var(--ink);margin-bottom:12px;padding-bottom:8px;border-bottom:2px solid var(--border);">
        大問 ${di+1}</div>`;
  ...
  html += `<button class="btn-primary" style="width:100%;margin-top:8px"
    onclick="Exam.saveAllAnswers()">回答を保存する</button>`;
  return html;
}
...
function renderSchemaEditor() {
  ...
  tempSchema.forEach((daimo, di) => {
    html += `<div style="background:var(--paper);border:1px solid var(--border);
      border-radius:6px;padding:14px;margin-bottom:14px;">
      <div style="font-family:'Zen Kaku Gothic New',sans-serif;font-weight:900;
        font-size:0.95rem;margin-bottom:10px;">大問 ${di+1}</div>`;
    daimo.questions.forEach((q, qi) => {
      html += `<div style="background:white;border:1px solid var(--border);
        ...
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
          ...
          <select onchange="Exam.updateQuestion(${di},${qi},'type',this.value)"
            style="padding:5px 8px;border:1.5px solid var(--border);border-radius:4px;
              font-family:inherit;font-size:0.8rem;background:white;outline:none;">
      ...
      if (q.type === 'choice') {
        html += `
          <select onchange="Exam.updateQuestion(${di},${qi},'format',this.value)"
            style="padding:5px 8px;border:1.5px solid var(--border);border-radius:4px;
              font-family:inherit;font-size:0.8rem;background:white;outline:none;">
            ${CHOICE_FORMATS.map(f=>`<option value="${f}" ${q.format===f?'selected':''}>${f.split(',').join('/')}</option>`).join('')}
          </select>
          <select onchange="Exam.updateQuestion(${di},${qi},'count',this.value)"
            style="padding:5px 8px;border:1.5px solid var(--border);border-radius:4px;
              font-family:inherit;font-size:0.8rem;background:white;outline:none;">
      ...
      html += `
          <button onclick="Exam.removeQuestion(${di},${qi})"
            style="margin-left:auto;padding:3px 10px;background:none;
              border:1px solid #ddd;border-radius:4px;cursor:pointer;
              font-size:0.74rem;color:#999;font-family:inherit;">削除</button>
        </div></div>`;
    });
    html += `<button onclick="Exam.addQuestion(${di})"
      style="width:100%;padding:8px;background:none;border:1px dashed var(--border);
        border-radius:4px;color:var(--ink-light);font-family:inherit;font-size:0.8rem;cursor:pointer;">
      ＋ 問題を追加</button></div>`;
  });
  el.innerHTML = html;
}

...
if (!el) return;
el.innerHTML = EXAM_SERIES.map(s => ` <div class="card" onclick="Exam.enterSeries('${s.id}','${esc(s.name)}')"> <div class="card-icon">${s.icon}</div> <div class="card-title">${esc(s.name)}</div> <div class="card-arrow">›</div> </div>`).join(’’);
}
